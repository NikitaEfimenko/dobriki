...
const selectColor = (value: string): string => {
  return getShortDayOfWeekFromDate(new Date()) === value
    ? "#29D70D"
    : "#929292";
};
  ...
  return (
    <g transform={`translate(${x},${y})`}>
      <text
        ...
        dy={16}
        textAnchor="middle"
        fill={selectColor(payload.value)}
      >
        {payload.value}
      </text>
    </g>
    ...
    >
      <div className="w-full h-[170px]">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart height={250} data={withMedian}>
            <XAxis
            ...
            <Bar dataKey="amt" barSize={28} radius={6}>
              {withMedian.map((entry, index) => (
                <Cell
                  key={`cell-${index}`}
                  fill={getColor(entry.amt, median)}
                />
              ))}
            </Bar>
            <Line
